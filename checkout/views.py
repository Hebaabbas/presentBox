...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        form_data = {
            ...
            'town_or_city': request.POST.get('town_or_city'),  # Added missing city field
            'street_address1': request.POST.get('street_address1'),
            'street_address2': request.POST.get('street_address2'),
            'county': request.POST.get('county'),  # Added missing county field
        }
        order_form = OrderForm(form_data)
        if order_form.is_valid():
            order = order_form.save(commit=False)
            pid = request.POST.get('client_secret', '').split('_secret')[0]
            order.stripe_pid = pid  # Assuming you store the Stripe PaymentIntent ID
    ...
    if not stripe_public_key:
        messages.warning(request, 'Stripe public key is missing. Did you forget to set it up?')
